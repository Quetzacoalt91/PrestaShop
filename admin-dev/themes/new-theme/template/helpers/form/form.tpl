...
<form id="{if isset($fields.form.form.id_form)}{$fields.form.form.id_form|escape:'html':'UTF-8'}{else}{if $table == null}configuration_form{else}{$table}_form{/if}{if isset($smarty.capture.table_count) && $smarty.capture.table_count}_{$smarty.capture.table_count|intval}{/if}{/if}" class="defaultForm form-horizontal{if isset($name_controller) && $name_controller} {$name_controller}{/if}"{if isset($current) && $current} action="{$current|escape:'html':'UTF-8'}{if isset($token) && $token}&amp;token={$token|escape:'html':'UTF-8'}{/if}"{/if} method="post" enctype="multipart/form-data"{if isset($style)} style="{$style}"{/if} novalidate>
	...
	{foreach $fields as $f => $fieldset}
		...
		<div class="card" id="fieldset_{$f}{if isset($smarty.capture.identifier_count) && $smarty.capture.identifier_count}_{$smarty.capture.identifier_count|intval}{/if}{if $smarty.capture.fieldset_name > 1}_{($smarty.capture.fieldset_name - 1)|intval}{/if}">
			{foreach $fieldset.form as $key => $field}
				...
				{elseif $key == 'input'}
					<div class="card-block form-wrapper">
					{foreach $field as $input}
						{block name="input_row"}
						<div class="form-group row{if isset($input.form_group_class)} {$input.form_group_class}{/if}{if $input.type == 'hidden'} hide{/if}"{if $input.name == 'id_state'} id="contains_states"{if !$contains_states} style="display:none;"{/if}{/if}{if isset($tabs) && isset($input.tab)} data-tab-id="{$input.tab}"{/if}>
						{if $input.type == 'hidden'}
							<input type="hidden" name="{$input.name}" id="{$input.name}" value="{$fields_value[$input.name]|escape:'html':'UTF-8'}" />
						{else}
							{block name="label"}
								{if isset($input.label)}
									<label class="col-form-label col-lg-3{if isset($input.required) && $input.required && $input.type != 'radio'} required{/if}">
										{if isset($input.hint)}
							...
							{block name="field"}
								...
								{if $input.type == 'text' || $input.type == 'tags'}
									...
									{foreach $languages as $language}
										...
										{if $languages|count > 1}
											...
											<div class="col-lg-2">
                                                                                                            <div class="dropdown">
                                                                                                                <button type="button" class="btn btn-sm btn-primary dropdown-toggle" tabindex="-1" data-toggle="dropdown">
                                                                                                                    {$language.iso_code}
                                                                                                                </button>
                                                                                                                <ul class="dropdown-menu">
                                                                                                                    {foreach from=$languages item=language}
                                                                                                                    <li><a class="dropdown-item" href="javascript:hideOtherLanguage({$language.id_lang});" tabindex="-1">{$language.name}</a></li>
                                                                                                                    {/foreach}
                                                                                                                </ul>
                                                                                                            </div>
											</div>
								...
								{elseif $input.type == 'textbutton'}
									...
									<div class="row">
										...
										<div class="col-lg-2">
											<button type="button" class="btn btn-primary{if isset($input.button.attributes['class'])} {$input.button.attributes['class']}{/if}{if isset($input.button.class)} {$input.button.class}{/if}"
										...
										<div class="col-lg-9">
											<div class="form-control-static row">
												<div class="col-xs-6">
													...
													<a href="#" id="addSwap" class="btn btn-primary btn-block">{l s='Add' d='Admin.Actions'} <i class="icon-arrow-right"></i></a>
												</div>
												<div class="col-xs-6">
													...
													<a href="#" id="removeSwap" class="btn btn-primary btn-block"><i class="icon-arrow-left"></i> {l s='Remove'}</a>
												</div>
											</div>
										</div>
									</div>
								...
								{elseif $input.type == 'switch'}
										...
										<input  class="custom-control-input" type="radio" name="{$input.name}"{if $value.value == 1} id="{$input.name}_on"{else} id="{$input.name}_off"{/if} value="{$value.value}"{if $fields_value[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}/>
									...
									{if isset($input.lang) AND $input.lang}
										{foreach $languages as $language}
											...
											{if $languages|count > 1}
												...
												<div class="col-lg-2">
                                                                                                                     <div class="dropdown">
                                                                                                                            <button type="button" class="btn btn-primary btn-sm dropdown-toggle" tabindex="-1" data-toggle="dropdown">
                                                                                                                                {$language.iso_code}
                                                                                                                            </button>
                                                                                                                            <ul class="dropdown-menu">
                                                                                                                                {foreach from=$languages item=language}
                                                                                                                                <li>
                                                                                                                                    <a class="dropdown-item" href="javascript:hideOtherLanguage({$language.id_lang});" tabindex="-1">{$language.name}</a>
                                                                                                                                </li>
                                                                                                                                {/foreach}
                                                                                                                            </ul>
                                                                                                                     </div>
								...
								{elseif $input.type == 'checkbox'}
									{if isset($input.expand)}
										<a class="btn btn-primary show_checkbox{if strtolower($input.expand.default) == 'hide'} hidden{/if}" href="#">
										...
										<a class="btn btn-primary hide_checkbox{if strtolower($input.expand.default) == 'show'} hidden{/if}" href="#">
								...
								{elseif $input.type == 'change-password'}
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name}-btn-change" class="btn btn-primary">
												<i class="icon-lock"></i>
												{l s='Change password...'}
											</button>
											<div id="{$input.name}-change-container" class="form-password-change well hide">
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														...
														<button type="button" id="{$input.name}-generate-btn" class="btn btn-primary">
												...
												<div class="row">
													<div class="col-lg-12">
														<button type="button" id="{$input.name}-cancel-btn" class="btn btn-primary">
			...
			{capture name='form_submit_btn'}{counter name='form_submit_btn'}{/capture}
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="card-footer text-lg-right">
						...
						<a href="{$back_url|escape:'html':'UTF-8'}" class="btn btn-primary" onclick="window.history.back();">
						...
						<button
							...
							class="{if isset($fieldset['form']['reset']['class'])}{$fieldset['form']['reset']['class']}{else}btn btn-primary{/if}"
							>
						...
						{foreach from=$fieldset['form']['buttons'] item=btn key=k}
							{if isset($btn.href) && trim($btn.href) != ''}
								<a href="{$btn.href}" {if isset($btn['id'])}id="{$btn['id']}"{/if} class="btn btn-primary{if isset($btn['class'])} {$btn['class']}{/if}" {if isset($btn.js) && $btn.js} onclick="{$btn.js}"{/if}>{if isset($btn['icon'])}<i class="{$btn['icon']}" ></i> {/if}{$btn.title}</a>
							{else}
								<button type="{if isset($btn['type'])}{$btn['type']}{else}button{/if}" {if isset($btn['id'])}id="{$btn['id']}"{/if} class="btn btn-primary{if isset($btn['class'])} {$btn['class']}{/if}" name="{if isset($btn['name'])}{$btn['name']}{else}submitOptions{$table}{/if}"{if isset($btn.js) && $btn.js} onclick="{$btn.js}"{/if}>{if isset($btn['icon'])}<i class="{$btn['icon']}" ></i> {/if}{$btn.title}</button>
							{/if}
						{/foreach}
						{/if}
					</div>
