...
class QueryListProvider implements ProviderInterface
{
    ...
    protected function createFilters(array $context, Operation $operation): Filters
    {
        ...
        // The API documentation (generated by ApiPlatform) advertises a 1-indexed "page" parameter,
        // so it must be converted to the offset actually used by the query, when no explicit offset is given.
        if ($offset === null && array_key_exists('page', $queryParameters)) {
            $offset = (max((int) $queryParameters['page'], 1) - 1) * ($limit ?? self::DEFAULT_PAGINATED_ITEM_LIMIT);
        }
        $paginationParameters = [
